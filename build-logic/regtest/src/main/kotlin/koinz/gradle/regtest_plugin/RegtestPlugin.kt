package koinz.gradle.regtest_plugin
...
class RegtestPlugin @Inject constructor(private val javaToolchainService: JavaToolchainService) : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        val startFirstSeedNodeTask = project.tasks.register<StartKoinzTask>("startRegtestFirstSeednode") {
        ...
        val startSecondSeedNodeTask = project.tasks.register<StartKoinzTask>("startRegtestSecondSeednode") {
            dependsOn(startBitcoindTask)
        ...
        val startMediatorTask = project.tasks.register<StartKoinzTask>("startRegtestMediator") {
            dependsOn(startFirstSeedNodeTask)
            ...
            mainClass.set("bisq.desktop.app.KoinzAppMain")
            arguments.set(
                createKoinzUserArgs(4444, ".localnet/mediator", "Mediator")
            )
        ...
        val startAliceTask = project.tasks.register<StartKoinzTask>("startRegtestAlice") {
            dependsOn(startFirstSeedNodeTask)
            dependsOn(startSecondSeedNodeTask)
            ...
            arguments.set(
                createKoinzUserArgs(5555, ".localnet/alice", "Alice", additionalArgs)
            )
        ...
        val startBobTask = project.tasks.register<StartKoinzTask>("startRegtest") {
            ...
            mainClass.set("bisq.desktop.app.KoinzAppMain")
            arguments.set(
                createKoinzUserArgs(6666, ".localnet/bob", "Bob")
            )
    ...
    private fun createKoinzUserArgs(
        nodePort: Int,
        dataDir: String,
        appName: String,
        additionalArgs: List<String> = emptyList()
    ): List<String> =
        createKoinzCommonArgs(nodePort) +
                listOf(
                    "--appDataDir=$dataDir",
                    "--appName=$appName"
                ) + additionalArgs
    ...
    private fun createKoinzCommonArgs(nodePort: Int): List<String> =
        listOf(
