...
package koinz.desktop.util;

import koinz.desktop.Navigation;
import koinz.desktop.app.BisqApp;
import koinz.desktop.components.AutoTooltipLabel;
import koinz.desktop.components.BisqScrollPane;
import koinz.desktop.components.BisqTextArea;
import koinz.desktop.components.InfoAutoTooltipLabel;
import koinz.desktop.components.indicator.TxConfidenceIndicator;
import koinz.desktop.components.paymentmethods.ArsBlueRatePopup;
import koinz.desktop.main.MainView;
import koinz.desktop.main.account.AccountView;
import koinz.desktop.main.account.content.fiataccounts.FiatAccountsView;
...
import koinz.core.app.BisqSetup;
...
import koinz.core.provider.price.MarketPrice;
import koinz.core.provider.price.PriceFeedService;
import koinz.core.trade.txproof.AssetTxProofResult;
...
import static koinz.desktop.util.FormBuilder.addTopLabelComboBoxComboBox;
...
public class GUIUtil {
    ...
    public final static TradeCurrency BSQ = CurrencyUtil.getTradeCurrency("KNZ").orElse(new CryptoCurrency("KNZ", "KNZ"));
    public static TradeCurrency TOP_ALTCOIN = CurrencyUtil.getTradeCurrency("XMR").orElse(new CryptoCurrency("XMR", "Monero"));
    ...
    public static boolean canCreateOrTakeOfferOrShowPopup(User user,
        ...
        if (currency != null && currency.getCode().equals("KNZ")) {
            return true;
        }
    ...
    public static int addRegionCountry(GridPane gridPane,
                                       int gridRow,
                                       Consumer<Country> onCountrySelectedHandler) {
        Tuple3<Label, ComboBox<koinz.core.locale.Region>, ComboBox<Country>> tuple3 = addTopLabelComboBoxComboBox(gridPane, ++gridRow, Res.get("payment.country"));

        ComboBox<koinz.core.locale.Region> regionComboBox = tuple3.second;
        regionComboBox.setPromptText(Res.get("payment.select.region"));
        regionComboBox.setConverter(new StringConverter<>() {
            @Override
            public String toString(koinz.core.locale.Region region) {
                return region.name;
            }

            @Override
            public koinz.core.locale.Region fromString(String s) {
                return null;
            }
        });
        ...
        regionComboBox.setOnAction(e -> {
            koinz.core.locale.Region selectedItem = regionComboBox.getSelectionModel().getSelectedItem();
    ...
    public static String getBsqInUsd(Price bsqPrice,
        ...
        Volume bsqAmountAsVolume = Volume.parse(bsqAmountAsString, "KNZ");
