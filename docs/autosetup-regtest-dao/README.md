...
The goal of these helper scripts is to set up a koinz DAO from scratch. The scripts run `bitcoind` in regtest mode for you and a `seed node`, `alice` and `bob` instances.
...
```
user@host:~/bin/KanoczTomas/koinz/docs/autosetup-regtest-dao$ ./start_bitcoind.sh
...
```
2. Open a new terminal (preferably a new tab), go to the `koinz` folder and start the `create_genesis.sh` script. The script tests for a running bitcoind node in regtest, so make sure step 1 was successful.
```
cd koinz
./create_genesis.sh
```
example output:
```
user@host:~/bin/KanoczTomas/koinz/docs/autosetup-regtest-dao/koinz$ ./create_genesis.sh
testing if bitcoind is running: bitcoind is running, all is ok!
Please provide Alice's KNZ Address
```
3. We need the KNZ address of `Alice`. Go to the `koinz` directory and start the `Alice` instance. Wait until the instance starts up then go to `DAO -> Receive` and copy the KNZ address. Note the address starts with a `B`. Open a new terminal and enter the commands below to start `Alice`. Then wait for the gui to start and paste the KNZ address to `create_genesis.sh` which is still running from step 2.
```
cd koinz
./alice
```
4. Enter the amount of BTC in the `create_genesis.sh` script you want `Alice` to receive as KNZ.
Example output:
```
user@host:~/bin/KanoczTomas/koinz/docs/autosetup-regtest-dao/koinz$ ./create_genesis.sh
testing if bitcoind is running: bitcoind is running, all is ok!
...
How much BTC to turn to KNZ for Alice? (sum must be 2.5BTC)
1
Alice will receive 1000000.00 KNZ to Bn2iUHwJQTreQaoajKwnT6h7pYF9nCWULJA
```
5. We need the KNZ address of `Bob`. Go to the `koinz` directory and start the `Bob` instance. Wait until the instance starts up then go to `DAO -> Receive` and copy the KNZ address. Note the address starts with a `B`. Open a new terminal and enter the commands below to start `Bob`. Then wait for the gui to start and paste the KNZ address to `create_genesis.sh` which is still running from step 2.
```
cd koinz
./bob
```
6. Enter the amount of BTC in the `create_genesis.sh` script you want `Bob` to receive as KNZ.
Example output:
```
tk@workbook:~/bin/KanoczTomas/koinz/docs/autosetup-regtest-dao/koinz$ ./create_genesis.sh
testing if bitcoind is running: bitcoind is running, all is ok!
Please provide Alice's KNZ Address
Bn2iUHwJQTreQaoajKwnT6h7pYF9nCWULJA
How much BTC to turn to KNZ for Alice? (sum must be 2.5BTC)
1
Alice will receive 1000000.00 KNZ to Bn2iUHwJQTreQaoajKwnT6h7pYF9nCWULJA
Please provide Bob's KNZ Address
BmoccyQEENPwxeZUdPAnDejEeTzykD9Kdbo
How much BTC to turn to KNZ for Bob? (sum must be 2.5BTC)
1.5
Bob will receive 1500000.00 KNZ to BmoccyQEENPwxeZUdPAnDejEeTzykD9Kdbo
...
8. Once confirmed the script will broadcast the genesis tx to the regtest blockchain and setup the `config.sh` file in the `koinz` directory which is used by the `seed_node`, `alice` and `bob` scripts. It populates the genesis txid and genesis block height so the instances can be launched accordingly.
Example output:
```
...
dao setup done
```
9. Start the `seed` node in full DAO node using the `seed_node` script. Make sure you are in the `koinz` directory.
```
cd koinz
./seed_node
```
10. Restart `Alice` and `Bob` you should see KNZ visible on the `DAO` page. Note the `seed node` is a full node and `alice`/`bob` are lite nodes in this setup.
...
1. Run `clean.sh`. It will clear the bitcoind files and koinz files from `~/.local/share/` directory. Make sure your `bitcoind/bitcoin-qt` is stopped before running.
```
user@host:~/bin/KanoczTomas/koinz/docs/autosetup-regtest-dao$ ./clean.sh
DANGER! This will nuke your bitcoin regtest dir and koinz seed, alice and bob regtest dirs as well!
Press Enter to continue, otherwise ctrl+c
cleaned bitcoind home and koinz seed, alice, bob instances. The conf file for koinz was reset to dao=false
```
